...
const onOff = async (onoff) => {
    ...
    for (step = 0; step <= numLeds/8; step++){
        // 107 /// 96
        const direction1 = onoff ? 0 : numLeds - (step + 0);
        const direction2 = onoff ? 203 : numLeds - (step + 203);
        const direction3 = onoff ? numLeds - (step + 311) : 311;
        const direction4 = onoff ? numLeds - (step + 514): 514; 
        const direction5 = onoff ? 622 : numLeds - (step + 622);

        leds.setColor(direction1, color);
        leds.setColor(direction2, color);
        leds.setColor(direction3, color);
        leds.setColor(direction4, color);
        leds.setColor(direction5, color);
        leds.update();
        await timer(2)
    }
}
...
const ledStrip = {
    ...
    getSaturation: () =>  {
        return ledsColor.saturation() * 100;
    },
    setHue: (hue) => {
